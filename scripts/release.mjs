#!/usr/bin/env node
/**
 * Release automation for dsh-click.
 *
 ...
 *      tag (message: `dsh-click v<version>`).
 *
 ...
 *
 * @module dsh-click/scripts/release
 */
...
// The helper-protocol handshake carries the version as a hardcoded string
// (tests/version.spec.ts trips when it drifts from package.json) — keep it
// in the same bump so the tripwire runs green inside the gate below.
...
git('tag', '-a', `v${version}`, '-m', `dsh-click v${version}`)
